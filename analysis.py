...
Produces (under ./plots/):
    ...
    plot2_propagation_beta.png
    residual_diagnostics_slide.png
"""
...
CSV_PATH = Path("ist_daten_2026_09_10.csv")
PLOTS_DIR = Path("plots")


def download_data(url: str, dest: Path) -> None:
    """Download the source CSV if it isn't already present locally.

    Sends a browser-like User-Agent: opentransportdata.swiss (like many
    CKAN-based portals) returns 403 Forbidden for the default urllib
    user agent, which looks like a bot to their server.
    """
    if dest.exists():
        print(f"Found existing {dest} ({dest.stat().st_size / 1e6:.1f} MB) — skipping download.")
        return
    ...
    try:
        with urllib.request.urlopen(request) as response, open(tmp_dest, "wb") as out_file:
        ...
        sys.exit(
            f"Failed to download the dataset from:\n  {url}\n"
            f"Error: {e}\n"
            f"If the URL has moved, get the current link for the "
            f"2026-09-10 ist-daten file from "
            f"https://data.opentransportdata.swiss/en/dataset/ist-daten-v2 "
            f"and re-run with that URL."
        )
...
# we take a look at a few lines to confirm delimiter and encoding
...
print(f"{csv_path}: {n_lines:,} lines")
...
# 2. Filtering to trains, real-time telemetry, and the Zürich-Chur corridor
# --------------------------------------------------------------------------
# cheap filters that apply universally
...
for name in corridor_names:
    matches = trains[
        trains["HALTESTELLEN_NAME"].str.contains(name, case=False, na=False, regex=False)
...
mask = trains["BPUIC"].isin(corridor_ids) & (trains["AN_PROGNOSE_STATUS"] == "REAL")
corr = trains.loc[mask].copy()
print(f"Corridor rows with REAL arrival: {len(corr):,}")

# Step 2: parse timestamps (first pass)
corr["scheduled_arr"] = pd.to_datetime(corr["ANKUNFTSZEIT"], format="%d.%m.%Y %H:%M", errors="coerce")
corr["actual_arr"] = pd.to_datetime(corr["AN_PROGNOSE"], format="%d.%m.%Y %H:%M", errors="coerce")

# Step 3: compute arrival delay in minutes (positive = late)
corr["arr_delay_min"] = (corr["actual_arr"] - corr["scheduled_arr"]).dt.total_seconds() / 60.0
...
# perhaps a time format mismatch -- inspect raw values
...
# 3. Delay computation
# --------------------------------------------------------------------------
# So the scheduled and actual times use different precisions -- re-parse
# with the correct format for each column.

corr["scheduled_arr"] = pd.to_datetime(corr["ANKUNFTSZEIT"], format="%d.%m.%Y %H:%M", errors="coerce")
corr["actual_arr"] = pd.to_datetime(corr["AN_PROGNOSE"], format="%d.%m.%Y %H:%M:%S", errors="coerce")
...
# Build train x station pivot: rows = trains, columns = corridor stations, values = arr_delay_min
...
print(f"Train x station delay matrix: {delay_matrix.shape}")
...
#
# For each segment k -> k+1 on the corridor, we fit a simple linear
# regression of the outgoing arrival delay on the incoming arrival delay:
#
#   y_i = alpha + beta * x_i + eps_i,  i = 1, ..., n
#
# where for train i: x_i is its arrival delay at station k (minutes),
# y_i is its arrival delay at station k+1 (minutes), and eps_i captures
# segment-specific noise (dwell variability, minor incidents, driver
# behaviour).
#
# beta (propagation coefficient): how much of the incoming delay carries
# forward. beta = 1 -> delay carries forward perfectly (pure carrier
# segment). beta = 0 -> delay is fully absorbed (buffer/slack in the
# schedule). beta < 1 -> partial absorption. beta > 1 -> delay is
# amplified on this segment.
#
# alpha (baseline accretion): how many minutes late even an on-time train
# (x_i = 0) arrives at the next station. alpha > 0 -> the schedule itself
# is slightly tight. alpha < 0 -> on-time trains catch up further.
#
# Estimation: ordinary least squares (OLS); alpha_hat and beta_hat
# minimise sum_i (y_i - alpha - beta * x_i)^2, with closed-form solution
# beta_hat = Cov(x, y) / Var(x), alpha_hat = ybar - beta_hat * xbar.
#
# r2: fraction of variance in outgoing delay explained by incoming delay.
# p: two-sided p-value for the null hypothesis beta = 0.
# se_beta: standard error of beta_hat; approximate 95% CI is
# beta_hat +/- 1.96 * se_beta.
#
# Implicit assumptions: (1) linearity of the incoming/outgoing delay
# relationship; (2) homoscedastic, independent, approximately Gaussian
# residuals (delay data has heavier tails, but sample sizes here are
# large enough for the coefficient estimates to be trustworthy);
# (3) correlations between trains sharing the same track window are not
# modelled -- a first-cut analysis.
#
# Regime split: we fit the regression on all trains (beta_all), on trains
# with incoming delay <= 3 min (beta_small, ~90th percentile), and on
# trains with incoming delay > 3 min (beta_large), to test whether
# propagation behaves differently for small jitter versus real delays.
# --------------------------------------------------------------------------
...
ax.set_title(
    f"Zürich HB ↔ Chur corridor, 10 Sept 2026 — {len(delays):,} arrival events",
    fontsize=13,
)

plt.tight_layout()
plot1_path = PLOTS_DIR / "plot1_delay_distribution.png"
plt.savefig(plot1_path, bbox_inches="tight", facecolor="white", dpi=200)
plt.close(fig)
...
# recompute se_beta so we can plot the 95% CI
# (already have this from linregress in the results table, but let's make sure)
se_by_segment = []
for k in range(len(corridor_names_ordered) - 1):
...
ax.text(
    len(plot_df) - 0.4, 1.03, "β = 1  (delay carries forward)",
    color="#C44E52", fontsize=10, ha="right", va="bottom",
)

ax.axhline(0.0, linestyle=":", color="#999", linewidth=1)
ax.text(
    len(plot_df) - 0.4, -0.03, "β = 0  (delay fully absorbed)",
...
for i, n in enumerate(plot_df["n_all"]):
    ax.text(i, ymin_pos, f"n = {n}", ha="center", va="top", fontsize=10, color="#555")
...
plt.tight_layout()
plot2_path = PLOTS_DIR / "plot2_propagation_beta.png"
plt.savefig(plot2_path, bbox_inches="tight", facecolor="white", dpi=200)
plt.close(fig)
...
segments_to_show = [
    (0, "Zürich HB → Thalwil"),    # absorbing, low R^2, high variability
    (5, "Bad Ragaz → Maienfeld"),  # carrier, high R^2, tight residuals
]
...
plt.tight_layout()
plot3_path = PLOTS_DIR / "residual_diagnostics_slide.png"
plt.savefig(plot3_path, bbox_inches="tight", facecolor="white", dpi=200)
plt.close(fig)
print(f"Saved {plot3_path}")

print("\nDone. Three plots written to ./plots/")
